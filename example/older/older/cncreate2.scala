...
object cncreate2 extends {
  ...
  def CardSave(target:html.Div) : Function1[MouseEvent,_]  = (e:dom.Event) => {
    ...
    val borders = target.getElementsByClassName("w3-border")
    for (tool <- borders) {
      //if (tool.asInstanceOf[dom.html.Div].className.contains("w3-border"))
      tool.asInstanceOf[dom.html.Div].className = tool.asInstanceOf[dom.html.Div].className.replaceAll("w3-border","")
    }
    val images = target.getElementsByTagName("img")
    for (tool <- images) {
      if (!tool.asInstanceOf[dom.html.Image].className.contains("awi-image"))
      tool.asInstanceOf[dom.html.Image].className += "awi-image"
    }

    val first = dom.window.document.getElementById("first-page").asInstanceOf[dom.html.Div]
  ...
  def PageAdd(target:html.Div) : Function1[Event,_]  = (e:dom.Event) => {
    ...
    val  theToolbar = "ToolNum" + pageNum
      pageNum += 1
    val editor = div( cls:="awi-editor w3-mobile w3-border").render
    val toolbar =  div( cls:="w3-mobile" , id:=theToolbar,
      select( cls:="ql-size",
    ...
    val pageBtn = div(cls:=" w3-mobile",pageAdd,imageAdd,pageDel).render
    val title = div(input("TITLE",cls:="w3-mobile w3-xlarge")).render
    val newPage = div(cls:="w3-cell-row", div(cls:="w3-cell",style:="position:relative;height:100px",editor),div(cls:="w3-cell kj-actbar",toolbar,pageBtn)).render
  ...
  def ImageAdd(target:html.Div) : Function1[Event,_]  = (e:dom.Event) => {
    ...
    val imgPage = div(cls:="w3-cell-row " , div(cls:="awi-image-page w3-cell w3-left" ,image),div(cls:="w3-cell kj-actbar",pageBtn)).render
